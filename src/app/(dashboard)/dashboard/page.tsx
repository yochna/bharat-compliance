...
export default async function DashboardPage() {
  ...
  try {
    user = await db.user.findUnique({
      ...
      include: {
        filings: { orderBy: { createdAt: "desc" }, take: 5 },
        documents: { orderBy: { createdAt: "desc" }, take: 3 },
      },
    })
  } catch (err) {
    console.error("Database unavailable, using fallback data:", err)
  }

  // Safe calculations
  const totalFiled = user?.filings?.reduce((sum, f) => sum + f.totalTaxableValue, 0) || 0
  const totalDocs = user?.documents?.length || 0
  const totalFilings = user?.filings?.length || 0
  ...
  const metrics = [
    {
      icon: TrendingUp,
      label: "Total GST Filed",
      value: formatAmount(totalFiled),
      footer: `${totalFilings} filing${totalFilings !== 1 ? "s" : ""} total`,
    ...
    },
    {
      ...
      label: "Next Deadline",
      value: "GSTR-3B",
      footer: "Due on 20th every month",
    ...
    },
    {
      ...
      footer: totalDocs > 0 ? "AI-generated documents" : "No documents yet",
      ...
      <div className="sticky top-0 z-10 flex items-center justify-between px-8 py-4 bg-[#0F1117]/80 backdrop-blur-xl border-b border-white/5">
        <div>
          ...
          <p className="text-[11px] text-white/30 mt-0.5 font-mono">
            FY 2025-26 · Q1 · {totalFilings} filing{totalFilings !== 1 ? "s" : ""} · {totalDocs} document{totalDocs !== 1 ? "s" : ""}
          </p>
        </div>
      ...
      <div className="flex-1 px-8 py-7 flex flex-col gap-6">
            ...
            <div className="px-5 py-2">
              {user?.filings && user.filings.length > 0 ? (
                ...
                <div className="py-10 text-center">
                  <p className="text-[13px] text-white/20 mb-3">No filings yet</p>
                  <a
                    href="/dashboard/gst"
                    className="text-[12px] text-[#FF5C1A] hover:text-[#FF7A1A] transition-colors no-underline"
                  >
        ...
        <div className="grid grid-cols-3 gap-4">
          ...
          >
            ...
            <p className="text-[11px] text-white/25 leading-relaxed">
              Generate GSTR-1 or GSTR-3B from your transactions instantly
            </p>
          </a>
          ...
          >
            ...
            <p className="text-[11px] text-white/25 leading-relaxed">
              AI-drafted legal notice or vendor agreement in EN or हिं
            </p>
          </a>
          ...
          >
            ...
            <p className="text-[11px] text-white/25 leading-relaxed">
              Query your compliance data in plain English or Hindi
            </p>
          </a>
        </div>
        ...
        <div className="bg-white/[0.02] border border-white/7 rounded-2xl p-6">
          <div className="flex items-center justify-between mb-6">
            ...
            <div className="text-right">
              <p className="text-[13px] font-bold text-[#4ade80]">
                {totalFilings > 0 ? "Active" : "No filings yet"}
              </p>
              <p className="text-[10px] text-white/25 font-mono">
                {totalFilings} filing{totalFilings !== 1 ? "s" : ""} recorded
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-4 gap-6">
            {[
              { label: "GST", pct: totalFilings > 0 ? 100 : 0, color: "#4ade80" },
              ...
              <div key={h.label}>
                ...
                <p className="text-[10px] text-white/20 mt-1.5">
                  {h.pct === 100 ? "All filed" :
                   h.pct >= 80 ? "Good standing" :
                   h.pct >= 60 ? "Needs attention" :
                   "Action required"}
                </p>
              </div>
            ))}
